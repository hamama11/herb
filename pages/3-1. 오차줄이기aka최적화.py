...
with st.expander("👉 직선의 기울기와 절편을 직접 조절해 보기", expanded=True):
    ...
    fig1 = px.scatter(x=x, y=y_linear, labels={"x": "x", "y": "y"}, title="데이터 vs 직선 모델")
...
with st.expander("👉 2차식 계수를 조절해 보기", expanded=False):
    ...
    fig2 = px.scatter(x=x, y=y_linear, labels={"x": "x", "y": "y"}, title="데이터 vs 2차식 모델")
...
# =========================
# Step 3. 다변수 회귀 (x1, x2 → y)
# =========================
st.header("🔹 Step 3. 다변수 회귀 (Multiple Regression)")
...
with st.expander("👉 w₁, w₂, b를 조절하면서 평면을 움직여 보기", expanded=False):
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    fig3 = px.scatter_3d(
        ...
        z="y",
        color="오차",
        title="다변수 회귀: 데이터 (점)와 예측값 (색으로 오차 표시)",
    )
...
# =========================
# Step 4. 비선형 회귀 (exp 형태)
# =========================
st.header("🔹 Step 4. 비선형 회귀 (Nonlinear Regression)")
...
with st.expander("👉 a, b를 조절해 비선형 곡선을 맞춰 보기", expanded=False):
    ...
    fig4 = px.scatter(x=x_nl, y=y_nl, labels={"x": "x", "y": "y"}, title="비선형 데이터 vs 모델")
...
# =========================
st.header("🔹 Step 5. 최적화의 공통 구조 (쉬운 버전)")

st.markdown("""
사실 우리가 한 일을 한 문장으로 정리하면 이렇게 말할 수 있습니다.

> **어떤 모양의 함수이든,  
> 오차(손실)를 줄이기 위해  
> 그 함수의 계수(파라미터)를 조절하는 과정이 바로 최적화다.**
