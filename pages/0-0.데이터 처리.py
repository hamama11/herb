...
with col_hero1:
    ...
    st.markdown(
        ...
        - 왼쪽/위쪽: **데이터 분석 6단계 흐름**  
        - 오른쪽/아래쪽: 온실 데이터를 이용한 **회귀·최적화 예시 요약**  

        세부 실습은 다른 페이지(예: `회귀.py`, `함수모를때_최적화.py`)에서 진행하고,  
...
# 3️⃣ 데이터 분리 시각화 (도넛형)
# -----------------------------
st.markdown("## 2. 데이터 분리: 훈련 vs 테스트 인포그래픽")

st.markdown(
    """
실제 분석에서는 한 데이터셋을 그대로 다 쓰지 않고  
**훈련(train)** / **테스트(test)** 로 나눠 씁니다.  
예시로 70:30 비율을 사용해 보겠습니다.
"""
)

train_ratio = st.slider("훈련 데이터 비율 (%)", 50, 90, 70, 5)
...
**훈련**: 모델이 패턴을 학습하는 데 사용  
**테스트**: 학습에 쓰지 않은 데이터로 **일반화 성능**을 확인
...
complexity = [1, 2, 3, 4, 5, 6]  # 개념적인 복잡도 점수 (임의)
...
- **함수모를때_최적화.py**:  
  - 실제 함수는 몰라도,  
    데이터를 통해 **이차 다항식/다변수 회귀**로  
    온실 환경의 최적점(온도·습도)을 추론
...
with col_uni:
    ...
    st.markdown(
        f"""
**추정된 모델식 (요약)**  
\\( \\hat y = {a2:.3f} x^2 + {a1:.3f} x + {a0:.3f} \\)  

- 온도만을 가지고 생장률을 설명  
- MSE ≈ **{mse_temp:.4f}**

➡ `함수모를때_최적화.py`의 1변수–1결과 회귀와 대응됩니다.
"""
    )
...
\\( \\hat y = {b_hat:.3f} + {w_temp:.3f} \\cdot \\text{{temp}} + {w_hum:.3f} \\cdot \\text{{humid}} \\)

- 온도와 습도를 **함께** 사용해 생장률을 설명  
- MSE ≈ **{mse_multi:.4f}**
...
- **중간**: 훈련/테스트 분리, 회귀모델 복잡도 스펙트럼  
- **아래**: 온실 데이터를 이용한  
  - 이차 다항 회귀 (온도 → 생장률)  
  - 다변수 회귀 (온도·습도 → 생장률)  

모든 내용의 공통된 한 문장 요약은 이것입니다 👇  

> 결국 우리는 **데이터에서 함수 모양을 추정(회귀)** 하고,  
> 그 함수 안에서 **오차(손실)를 최소화하도록 계수(파라미터)를 조절**하는  
> **최적화 과정을 수행**하고 있다.
"""
)
